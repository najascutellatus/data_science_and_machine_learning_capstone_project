...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, 
                     names='class', 
                     title=f'Total Successful Launches: {entered_site}')
        return fig
...
def get_pie_chart(entered_site, payload):
    filtered_df = spacex_df
    if entered_site == 'ALL':
        filtered_df=filtered_df.loc[filtered_df['Payload Mass (kg)'].between(payload[0], payload[1])]
        ...
        filtered_df=filtered_df.loc[
            ...
            & 
            (filtered_df['Payload Mass (kg)'].between(payload[0], payload[1]))
            ]
        fig = px.scatter(filtered_df,
                         x='Payload Mass (kg)',
                         y='class',
